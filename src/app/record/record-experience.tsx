...
const RecordExperiencePage = ({
  ...
  const handleSubmit = async () => {
    ...
    console.log(
      ...
      experienceText,
      "Type:",
      typeof experienceText,
      "selectedEmotion:",
      selectedEmotion,
      "Type:",
      typeof selectedEmotion,
      "emotionLabel:",
      emotions.find((e) => e.id === selectedEmotion)?.label,
      "Type:",
      typeof emotions.find((e) => e.id === selectedEmotion)?.label
    );
    ...
    try {
      const response = await fetch(
        `https://gunanana.onrender.com/api/${experienceId}/record`,
        {
          method: "PUT",
          headers: {
          ...
          >
            ...
            {showTip && (
              <div className="tip-container">
                [Tip] 육하 원칙(누가, 언제, 어디서, 무엇을, 어떻게, 왜)를
                사용하면 좀 더 명확한 경험을 작성할 수 있어요!
              </div>
            )}
          </div>
        </div>
